  ...
  (define value-of
    (lambda (exp0 env) 
      (cases expression exp0
        ...
        (var-exp (var)
                 (let* [(ref (apply-env var env)) (val (deref ref))]
                   (if (trunk? val)     ;;trunk for call-by-need
                         (let [(v (val-trunk val))]
                           (begin (setref! ref v) v));;;memoization
                         val)))
...
(define val-trunk
  (lambda (trunk)
    (let [(exp (trunk->exp trunk)) (env (trunk->env trunk))]
      (value-of exp env))))
...
(define trunk?
  (lambda (trunk)
    (and (list? trunk) (equal? (list-ref trunk 0) 'trunk))))
